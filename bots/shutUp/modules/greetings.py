...
@Client.on_message(
    filters.command(["setwelcome", f"setwelcome@{bot_username}"])
    & filters.group
    & isModuleToggledFilter("greetings"),
)
async def setWelcome(client: Client, message: Message):
    if message.reply_to_message:
        ...
        await message.reply_text(
            f"Successfully set the welcome message to: {' '.join(message.command[1:])}",
        )


@Client.on_message(
    filters.command(["setbye", f"setbye@{bot_username}"])
    & filters.group
    & isModuleToggledFilter("greetings"),
)
async def setBye(client: Client, message: Message):
    if message.reply_to_message:
        ...
        await message.reply_text(
            f"Successfully set the goodbye message to: {' '.join(message.command[1:])}",
        )

...
@Client.on_message(
    filters.command(["shut", f"shut@{bot_username}"])
    & filters.group
    & isModuleToggledFilter("shut"),
    group=0,
)
async def shut(client: Client, message: Message):
    if (
        await message.chat.get_member(message.from_user.id)
    ).privileges.can_delete_messages or message.from_user.id == owner_id:
        DB.groups.toggleShut(message.chat.id)
    else:
        await message.reply_text(
            'You need delete permission in this group to be able to toggle "shut"',
        )


@Client.on_message(
    ~filters.me & filters.group & isShut & isModuleToggledFilter("shut"),
    group=0,
)
